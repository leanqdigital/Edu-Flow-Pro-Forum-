...
document.getElementById("submit-post").addEventListener("click", async (e) => {
  ...
  console.log("temp post is ", tempPost);
  ...
  let fileData = null;
  if (uploadedFile) {
    const fileFields = [{ fieldName: "file__content", file: uploadedFile }];
    const toSubmitFields = {};
    await processFileFields(toSubmitFields, fileFields, awsParam, awsParamUrl);
    fileData =
      typeof toSubmitFields.file__content === "string"
        ? JSON.parse(toSubmitFields.file__content)
        : toSubmitFields.file__content;
    ...
    const response = await ApiService.query(
        ...
        `,
      {
        payload: {
          ...
          related__course_x_id: courseID,
          file__type: uploadedFile ? fileType : null,
  ...
  try {
    const fetchResponse = await ApiService.query(
      ...
      arg: ["disable__new__comments"]
          ...
          Post_Publish_Date: field(arg: ["created_at"])
        }
      }
      `,
      { id: newPost.id }
    ...
    postElement.querySelector(".load-comments-btn").dataset.postId =
      actualPost.ID;
    postElement.querySelector(".bookmark-button").postId = actualPost.ID;
