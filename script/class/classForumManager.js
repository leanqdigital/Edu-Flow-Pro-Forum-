// this file contains all the posts fetcher code. . .
...
class ForumManager {
  ...
  buildQuery() {
    ...
    filters.push(
      `
      { where: { Related_Course_X: [{ where: { id: "${courseID}" } }] } }
   ...
   query calcForumPosts($limit: IntScalar, $offset: IntScalar${
     this.needsUserId() ? ", $id: EduflowproContactID" : ""
   }${
   ...
   {
  calcForumPosts(${argsString}) {
  ID: field(arg: ["id"])
    Author_ID: field(arg: ["author_id"])
    Author_First_Name: field(arg: ["Author", "first_name"])
    Author_Last_Name: field(arg: ["Author", "last_name"])
    Author_Profile_Image: field(
      arg: ["Author", "profile_image"]
    )
    Date_Added: field(arg: ["created_at"])
    Post_Title: field(arg: ["post_title"])
    Post_Copy: field(arg: ["post_copy"])
    Featured_Post: field(arg: ["featured__post"])
    File_Type: field(arg: ["file__type"])
    File_Content: field(arg: ["file"])
    Disable_New_Comments: field(
      arg: ["disable__new__comments"]
    )
    Author_Forum_Image: field(
      arg: ["Author", "profile_image"]
    )
    Author_Display_Name: field(
      arg: ["Author", "display_name"]
    )
    Post_Publish_Date: field(arg: ["last_modified_at"])
        ForumCommentsTotalCount: countDistinct(
      args: [{ field: ["ForumComments", "id"] }]
    )
    Member_Post_Upvotes_DataTotal_Count: countDistinct(
      args: [{ field: ["Member_Post_Upvotes_Data", "id"] }]
    )
    ForumCommentsIDCalc: field(arg: ["ForumComments", "id"])
 ...
 mutation deleteForumPost($id: EduflowproForumPostID) {
    ...
    try {
      const query = `
        ...
        calcContacts(
          ...
          limit: $limit
          offset: $offset
          orderBy: [{ path: ["last_modified_at"], type: desc }]
        ) {
        ID: field(
        arg: ["Saved_Post_Xes_Data", "id"]
        )
        Saved_Post_ID: field(arg: ["Saved_Post_Xes", "id"])
        Contact_ID: field(
        arg: ["Saved_Post_Xes_Data", "Contact_X", "id"]
        )
        }
      }
      `;
      ...
      //   ) {
      //     ID: field(
      //       arg: ["Saved_Posts_Data", "id"]
      //     )
      //     Saved_Post_ID: field(arg: ["Saved_Posts", "id"])
      //     Contact_ID: field(
      //       arg: ["Saved_Posts_Data", "contact_id"]
      //     )
      //   }
      // }
      // `;
      ...
      if (!data || !data.calcContacts) {
        return;
      }

      this.savedPostIds.clear();
      data.calcContacts.forEach(({ ID, Saved_Post_ID }) => {
  ...
  async fetchAllBookmarkIdsForPost(postId) {
    ...
    const query = `
  query calcOContactXSavedPostXes {
  calcOContactXSavedPostXes(
    query: [
      { where: { Contact_X: [{ where: { id: ${this.userId} } }] } }
      { andWhere: { Saved_Post_X: [{ where: { id: ${postId} } }] } }
    ]
  ) {
    ID: field(arg: ["id"])
  }
    ...
    const data = await ApiService.query(query);
    return (
      data?.calcOContactXSavedPostXes?.map((bookmark) => bookmark.ID) || []
    );
  }
  ...
  async createBookmark(postId) {
    ...
    const query = `
mutation createOContactXSavedPostX(
  $payload: OContactXSavedPostXCreateInput = null
) {
  createOContactXSavedPostX(payload: $payload) {
    contact_x_id
    saved__post_x_id
  }
}
  `;
    const variables = {
      payload: {
        contact_x_id: this.userId,
        saved__post_x_id: postId
      }
    };

    const response = await ApiService.query(query, variables);
    return response.createOContactXSavedPostX.id;
  }
  ...
  async deleteBookmark(savedRecordId) {
    // const query = `
    //     mutation deleteOContactSavedPost($id: EduFlowproOContactSavedPostID) {
    ...
    console.log("delete bookmark code is running.");

    const query = `mutation deleteOContactXSavedPostX(
  $id: EduflowproOContactXSavedPostXID
) {
  deleteOContactXSavedPostX(
    query: [{ where: { id: $id } }]
  ) {
    id
  }
...
`;
    ...
    const response = await ApiService.query(query, variables);
    if (!response || !response.deleteOContactXSavedPostX) {
...
`;
  ...
  async fetchPostVoteCount(postId) {
    console.log("fetching the updated vote counts now ❤️❤️");
    //     const query = `query calcForumPosts($id: EduflowproForumPostID) {
    //   calcForumPosts(query: [{ where: { id: ${postId} } }]) {
    //     Member_Post_UpvotesTotal_Count: countDistinct(
    //       args: [{ field: ["Member_Post_Upvotes_Data", "id"] }]
    //     )
    ...
    const query = `query calcForumPosts {
  calcForumPosts(query: [{ where: { id: ${postId} } }]) {
    Member_Post_UpvotesTotal_Count: countDistinct(
      args: [{ field: ["Member_Post_Upvotes_Data", "id"] }]
    )
...
`;
    ...
    return response.calcForumPosts[0].Member_Post_UpvotesTotal_Count;
  }
  ...
  async toggleVote(postId) {
    ...
    try {
      ...
      if (isVoted) {
        ...
        console.log("the vote id is", voteId);
        if (!this.votedPostIds.has(postId)) {
          this.votedPostIds.set(postId, new Set());
        }
  ...
  async createVote(postId) {
    ...
    console.log(
      "returning the vote id now from this repsonse",
      response?.createMemberPostUpvotesPostUpvotes?.id
    );

    // return response.data.createMemberPostUpvotesPostUpvotes.id;
    return response?.data?.createMemberPostUpvotesPostUpvotes?.id;
  }
  ...
  async deleteVotes(postId) {
    ...
    await Promise.all(
      [...voteIds].map((id) =>
        ApiService.query(
          `
                  mutation deleteVote($id: EduflowproMemberPostUpvotesPostUpvotesID) {
  ...
  async fetchComments(postId) {
    console.log("this is the fn being run after i click in comments .. .");
    try {
      const query = `
          query {
          calcForumComments(
         query: [
          {
          where: { Forum_Post: [{ where: { id: ${postId} } }] }
            }
           {
            andWhere: {
             ...
             orderBy: [{ path: ["last_modified_at"], type: asc }]
             ) {
            ID: field(arg: ["id"])
            Author_First_Name: field(arg: ["Author", "first_name"])
            Author_Last_Name: field(arg: ["Author", "last_name"])
            ...
            Member_Comment_Upvotes_DataTotal_Count: countDistinct(
            args: [{ field: ["Member_Comment_Upvotes", "id"] }]
            )
            File_Type: field(arg: ["file__type"])
            File_Content: field(arg: ["file"])
            Author_Forum_Image: field(
  ...
  async createComment(postId, content, mentions, fileType, uploadedFile) {
    ...
    try {
      ...
      //     Mentions_Contact_ID: mentions.map((id) => ({ id: Number(id) })),
      //     file__type: fileType,
      //     file: fileData
      //   }
      // };
...
`;
      ...
      const variables = {
        payload: {
          ...
          author_id: this.userId,
          forum_post_id: postId,
          Mentions: mentions.map((id) => ({ id: Number(id) }))
        }
      };
    ...
    try {
      ...
      //     const fetchQuery = `
      //       query calcForumComments($id: EduflowproForumCommentID) {
      ...
      const fetchQuery = `
        ...
        ID: field(arg: ["id"]) 
        Author_First_Name: field(arg: ["Author", "first_name"])
        Author_Last_Name: field(arg: ["Author", "first_name"])
        Author_Profile_Image: field(
        arg: ["Author", "profile_image"]
        )
        Author_ID: field(arg: ["author_id"])
  ...
  async fetchVoteForComment(commentId) {
    try {
      ...
      query calcMemberCommentUpvotesForumCommentUpvotesMany(
        $member_comment_upvote_id: EduflowproContactID
        $forum_comment_upvote_id: EduflowproForumCommentID
      ) {
    ...
    try {
      const query = `
        query {
          ...
          query: [{
            where: { Reply_to_Comment: [{ where: { id: "${commentId}" } }] }
          }]) {
           ID: field(arg: ["id"])
          Author_First_Name: field(arg: ["Author", "first_name"])
          Author_Last_Name: field(arg: ["Author", "last_name"])
      ...
      //           reply_to_comment_id: [{ where: { id: null } }]
      //         }
      //       }]
      //     ) {
      //       ID: field(arg: ["id"])
  ...
  async createReply(commentId, content, mentions, fileType, uploadedFile) {
    console.log("THIS RUNS WHEN WE WANNA REPLY A COMMENT.");
      ...
      if (buttonForComment) {
        ...
        try {
          const response = await ApiService.query(
            `
          query calcForumPosts($id: EduflowproForumPostID) {
          ...
          Post_Status: field(arg: ["post_status"])
            ...
            //POST PUBLISH DATE HAS BEEN REMOVED CURRENTLY AS THE FIELD IS NOT PRESENT
            //SCHEMA ABOVE . . .WILL CREATE LATER. .
            //         const response = await ApiService.query(
            //           `
            //  query calcForumPosts($id: EduflowproForumPostID) {
            //       calcForumPosts(query: [{ where: { id: $id } }]) {
            ...
            //         Author_Display_Name: field(arg: ["Author", "display_name"])
            //         Post_Status: field(arg: ["post_status"])
            //         Post_Publish_Date: field(arg: ["post__publish__date"])
            //       }
            //     }
            //             `,
            { id: postId }
